...
class ModuleData extends Component {
    ...
    getDataSlice = (index, data) => {
        const dataSlice = {};
        index
            .search("node:" + this.props.selectedNode)
        ...
        this.setState(
            { dataSlice: dataSlice, changeFlag: Math.random() },
            () => {
                this.forceUpdate();
            }
        );
            ...
            if (aggregate === false) {
                ...
                Object.keys(data).forEach((key, index) => {
                    values.push({
                        ...
                        values: [
                            key,
                            this.buildLocationString(data[key].loc),
                            data[key][field]
                        ]
                    });
                });
                ...
                Object.keys(data).forEach(key => {
                    ...
                    subFields.forEach(subField => {
                        if (subField in data[key][field]) {
                            ...
                            if (
                                !isNaN(
                                    data[Object.keys(data)[0]][field][subField]
                                )
                            ) {
                                vals.push(NaN);
                            } else {
