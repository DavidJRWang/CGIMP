...
class SettingsDialog extends React.Component {
    ...
    componentDidMount() {
        ...
        this.props.parentState.mapConfig.toolTips.forEach(tipField => {
            ...
            let fs;
            "fs" in tipField ? (fs = tipField.fs) : (fs = "");

            toolTips.push({
                field: tipField.field,
                type: tipField.type,
                label: label,
                fs: fs
            });
        ...
        this.props.parentState.dataPanelConfig.dataFields.forEach(field => {
            let title;
            "title" in field ? (title = field.title) : (title = field.field);
            let fs;
            "fs" in field ? (fs = field.fs) : (fs = "");
        ...
        this.setState({ toolTips: toolTips, moduleTables: moduleTables });
    }
    ...
    addToolTip = event => {
        const blankToolTip = {
            field: this.props.parentState.nodeFields[0],
            type: nodeTypes[0],
    ...
    addModuleTable = event => {
        const blankModuleTable = {
            field: this.props.parentState.moduleFields[0],
            type: moduleTypes[0],
            aggregate: false,
            title: "",
            fs: ""
    ...
    render() {
                                ...
                                return (
                                    <ListItem key={"tip" + index.toString()}>
                                        <ToolTipPane
                                            ...
                                            selectedField={
                                                this.state.toolTips[index].field
                                            }
                                            types={nodeTypes}
                                            selectedType={
                                                this.state.toolTips[index].type
                                            }
                                            label={
                                                this.state.toolTips[index].label
                                            }
	     ...
	     */}
                            ...
                            <ListItem key={Math.random().toString()}>
                                <IconButton
                                    className={classes.button}
                                    aria-label="Add Module Table"
                                    color="inherit"
                                    onClick={this.addModuleTable}
                                >
                                    <AddIcon />
                                </IconButton>
                            </ListItem>
...
const ModuleDataTable = ({
    ...
    return (
        <div>
            ...
            <IconButton
                className={classes.button}
                aria-label="Delete Module Table"
                color="inherit"
                onClick={remove("moduleTables", index)}
            >
                <DeleteIcon />
            </IconButton>
        </div>
