...
function drawKey(svg, x, y, dataRange, title, precision) {
    ...
    svg.append("text")
        .text(function() {
            return round(dataRange[0], precision);
        })
        .attr("transform", "translate(" + xPos + ", " + yPos + ")");
    yPos = y + 85;
    svg.append("text")
        .text(function() {
            return round((dataRange[1] - dataRange[0]) / 2, precision);
        })
        .attr("transform", "translate(" + xPos + ", " + yPos + ")");
    ...
    svg.append("text")
        .text(function() {
            return round(dataRange[1], precision);
        })
        .attr("transform", "translate(" + xPos + ", " + yPos + ")");
}
...
class SvgModuleMap extends Component {
    ...
    updateParentState = () => {
        if (this.props.selectedNode) {
            let thisPod = d3.select(
                "[name=pod-" + this.props.selectedNode + "]"
            );
    ...
    handleClick = data => {
        if (this.props.selectedNode) {
            let oldPod = d3.select(
                "[name=pod-" + this.props.selectedNode + "]"
            );
            oldPod.attr("stroke", "#000000").attr("stroke-width", "0.5");
        }
        let thisPod = d3.select("[name=" + data.name + "]");
        thisPod.attr("stroke", "#00FF00").attr("stroke-width", "2");
    ...
    d3DrawGrid = () => {
        ...
        const svgWidth = dim[0] * podHeight + 50,
            svgHeight = dim[1] * podWidth;
